...
public class SlotApi : ApiEndpoint, ICarterModule
{
    ...
    public void AddRoutes(IEndpointRouteBuilder app)
    {
        ...
        group1.MapGet(string.Empty, GetSlotsByMentorId);
    }

    private static async Task<IResult> GetSlotsByMentorId(ISender sender, Guid mentorId, string? Date)
    {
        var result = await sender.Send(new Query.FindSlotsByMentorId(mentorId, Date));
