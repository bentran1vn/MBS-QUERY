...
public class FindUserByIdQueryHandler : IQueryHandler<Query.FindUserById, Reponse.MemberDetail>
{
    ...
    public async Task<Result<Reponse.MemberDetail>> Handle(Query.FindUserById request,
        CancellationToken cancellationToken)
    {
        var user = await _userRepository.FindByIdAsync(request.Id, cancellationToken);
        if (user == null) return (Result<Reponse.MemberDetail>)Result.Failure(new Error("404", "User not found"));
